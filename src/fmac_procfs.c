...
static int fmac_proc_show(struct seq_file *m, void *v) {
  ...
  hash_for_each_rcu(fmac_rule_ht, bkt, rule, node) {
    seq_printf(m, "  [Bucket %d] Path: %s, UID: %u, Deny: %d\n", bkt,
               rule->path_prefix, rule->uid, rule->deny);
  }
  rcu_read_unlock();
...
static ssize_t fmac_proc_write(struct file *file, const char __user *buffer,
  ...
  if (sscanf(kbuf, "add %255s %u %d", path, &uid, &deny) == 3) {
    if (deny != 0 && deny != 1) {
      fmac_append_to_log("[FMAC] Invalid deny value: %d. Must be 0 or 1.\n",
                         deny);
      return -EINVAL;
    }
    fmac_add_rule(path, (uid_t)uid, (bool)deny);
    ...
    fmac_append_to_log("[FMAC] Invalid command. Use: 'add /path uid deny' or "
                       "'printk_on/off'.\n");
    return -EINVAL;
  }
