...
const std::string SigCheck::DEBUG_EXPECTED_SIGNATURE =
    "1692015C04AA6CEA61B9E3FBF6CBC9FA5933E6A6A5788C12D1289A6D9E51D45E";
const std::string SigCheck::RELEASE_EXPECTED_SIGNATURE =
    "1C9CEAC6A82DE20EF909103926C296B2882653B4C9189360FCA9F081FCD663B1";
...
bool SigCheck::validate(const std::string &apkPath) {
  ...
  try {
    ...
    if (magicBuf != APK_SIGNING_BLOCK_MAGIC)
      return false;
    ...
    while (sCursor < sEnd) {
      ...
      std::vector<uint8_t> signedData =
          getLengthPrefixedSlice(&signerPtr, signerEnd);
      ...
      std::vector<uint8_t> certsWrap = getLengthPrefixedSlice(
          &certsPtr, signedData.data() + signedData.size());
      ...
      while (cPtr < cEnd) {
        ...
        if (currentSig == DEBUG_EXPECTED_SIGNATURE ||
            currentSig == RELEASE_EXPECTED_SIGNATURE) {
          return true;
        }
      }
    }
...
std::vector<uint8_t> SigCheck::getLengthPrefixedSlice(const uint8_t **cursor,
                                                      const uint8_t *end) {
  if (*cursor + 4 > end)
    return {};
